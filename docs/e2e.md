...
| 2026-09-17 | v1.1.0 (stack updated in place) | bc0ff7978 | S2 provisioning smoke PASS on the second attempt. Verify `enabled: true`, `warnings: []`; both DryRun probes `denied` (`create_subnet_default_vpc` 59df6337-9f0a-4323-a872-be6c311eee2b, `run_instances_denied_type` m5.large c4fca398-c665-46fc-b96c-2a6c29a396b6) — the fence denies as well as allows. All ten pinned quota codes answered live, including the three read back from Service Quotas for this release. Encrypted root volume, network and instance adopted on repeat, stop / Elastic IP / start with the EIP as the public IP, `describe_managed` empty afterwards. The first attempt failed its network cleanup with `DisassociateRouteTable: UnauthorizedOperation` and stranded a VPC: core-api disassociated after deleting the subnets, and a tag-conditioned call on a resource AWS can no longer find fails closed rather than answering NotFound. Fixed in core-api (costpilot#329), not in the template. |
