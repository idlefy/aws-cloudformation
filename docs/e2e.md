...
| 2026-09-17 | v1.1.0 (unchanged) | 83bb72c7f | First run after core-api started gating launches on the capability verify detects on the DEPLOYED stack instead of the version it offered (costpilot#330). The provisioning block verified earlier the same day carried no capability, so launch would have been refused; verify was re-run once and the `ec2:GetEbsEncryptionByDefault` probe was permitted: `capabilities: {"ebs_encryption": true}`, `warnings: []`. Smoke then PASS on the first attempt with `run_instance` going through that gate: all ten quota codes, network and instance adopted on repeat, encrypted root volume (`vol-0d9260fd99c48251b`, account KMS key), stop / Elastic IP / start, full cleanup. SSH ingress was `203.0.113.10/32` (TEST-NET-3, unroutable) — the smoke never connects to the box. Cleanup confirmed independently with an admin profile, not only by the provision role's own inventory: instance `i-063d25174351e225f` terminated, its volume and network interface NotFound, no Elastic IP, VPC, internet gateway or security group tagged `IdlefyManaged` left. The Resource Groups Tagging API still listed the three deleted ARNs for a while afterwards; it lags, so it is not a cleanup check. |
